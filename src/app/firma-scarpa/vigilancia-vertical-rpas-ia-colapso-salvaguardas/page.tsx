...
export async function generateStaticParams() {
  const mdxPosts = getAllPosts().filter(
    (p) => (p.frontmatter.category || "").toLowerCase() === "firma-scarpa" || (p.frontmatter.section || "").toLowerCase() === "firma-scarpa",
  );
...
export async function generateMetadata({
  ...
  const canonical = `https://www.derechoartificial.com/firma-scarpa/${slug}`;
  const metaDescription =
    mdxPost.excerpt ||
    description ||
    "Análisis crítico e independiente sobre la intersección del Derecho, la Ética y la Inteligencia Artificial.";
...
export default async function FirmaScarpaSlugPage({
  ...
  const jsonLd = createArticleJsonLd({
    url: `https://www.derechoartificial.com/firma-scarpa/${slug}`,
    headline: title,
    ...
    <>
      ...
      <Breadcrumbs
        items={[
          ...
          { label: title, href: `/firma-scarpa/${slug}` },
        ]}
      />
      <LegalLayout
        title={title}
