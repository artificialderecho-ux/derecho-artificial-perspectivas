...
type ScrollNavProps = { active: number; total: number; onClick: (index: number) => void }

export function ScrollNav({ active, total, onClick }: ScrollNavProps) {
  return (
...
type ChapterCardProps = {
  ...
  index: number;
  isActive: boolean;
  chapterRef: (element: HTMLElement | null, index: number) => void;
}

export function ChapterCard({ chapter, index, isActive: _isActive, chapterRef }: ChapterCardProps) {
  ...
  return (
    <section
      ref={(el) => { cardRef.current = el; chapterRef(el, index) }}
      id={`chapter-${index}`}
      style={{
        minHeight: '90vh',
        display: 'flex',
