      ...
      <section className="pt-20 pb-12 bg-muted/30 border-b border-border/50">
        <div className="container-narrow text-center">
          ...
          <div className="flex items-center justify-center gap-4 text-sm text-muted-foreground font-sans">
            {date && (
              <>
                <time dateTime={date}>{new Date(date).toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' })}</time>
            ...
            <div className="flex items-center gap-2">
              <span>Por</span>
              {author.href ? (
                <Link href={author.href} className="font-medium text-foreground hover:text-primary transition-colors underline underline-offset-4 decoration-primary/30 hover:decoration-primary">
                  {author.name}
                </Link>
              ) : (
